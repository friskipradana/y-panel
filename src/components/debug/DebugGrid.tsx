...
const DEBUG_GRID_PRESET_COLORS = [
  cssVar('--debug-grid-preset-cyan', '#00ffff'),
  cssVar('--debug-grid-preset-magenta', '#ff00ff'),
  cssVar('--debug-grid-preset-green', '#00ff00'),
  cssVar('--debug-grid-preset-red', '#ff0000'),
  cssVar('--debug-grid-preset-yellow', '#ffff00'),
  cssVar('--debug-grid-preset-white', '#ffffff'),
]
...
export function DebugGrid() {
  ...
  const [settings, setSettings] = useState<GridSettings>({
    ...
    gutter: 16,
    opacity: 0.3,
    color: cssVar('--debug-grid-color', '#00ffff'),
    ...
    <>
                ...
                {settings.showMeasurements && (
                  <>
                    ...
                    <div style={{
                      ...
                      background: 'var(--debug-grid-measure-bg)',
                      padding: '2px 4px',
                      borderRadius: 2,
                      whiteSpace: 'nowrap',
                    }}>
      ...
      {showInfo && enabled && (
        <div style={{
          ...
          top: 20,
          right: 20,
          background: 'var(--debug-grid-panel-bg)',
          ...
          <div style={{ marginBottom: 8, paddingBottom: 8, borderBottom: `1px solid ${hexToRgba(settings.color, 0.2)}` }}>
            <div style={{ color: 'var(--debug-grid-muted-text)', fontSize: 9, marginBottom: 4 }}>{t('debug.gridStatus')}</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.type')}:</span>
              <span style={{ fontWeight: 700, textTransform: 'uppercase' }}>{gridType}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.state')}:</span>
          ...
          {/* Screen Info */}
          <div style={{ marginBottom: 8, paddingBottom: 8, borderBottom: `1px solid ${hexToRgba(settings.color, 0.2)}` }}>
            <div style={{ color: 'var(--debug-grid-muted-text)', fontSize: 9, marginBottom: 4 }}>{t('debug.screenInfo')}</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.viewport')}:</span>
              <span>{screenInfo.width}×{screenInfo.height}px</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.ratio')}:</span>
              <span>{(screenInfo.width / screenInfo.height).toFixed(2)}:1</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.dpr')}:</span>
              <span>{screenInfo.dpr}x</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.center')}:</span>
          ...
          {/* Grid Metrics */}
          <div style={{ marginBottom: 8, paddingBottom: 8, borderBottom: `1px solid ${hexToRgba(settings.color, 0.2)}` }}>
            <div style={{ color: 'var(--debug-grid-muted-text)', fontSize: 9, marginBottom: 4 }}>{t('debug.gridMetrics')}</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.columns')}:</span>
              <span>{settings.columns}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.rows')}:</span>
              <span>{settings.rows}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.gutter')}:</span>
              <span>{settings.gutter}px</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.colWidth')}:</span>
              <span>{Math.round(columnWidth)}px</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.rowHeight')}:</span>
          ...
          <div style={{ 
            fontSize: 9,
            color: 'var(--debug-grid-subtle-text)',
            lineHeight: 1.4,
          }}>
      ...
      {showSettings && (
        <div style={{
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.opacity')}</span>
          ...
          {/* Color Presets */}
          <div style={{ marginBottom: 10 }}>
            <div style={{ color: 'var(--debug-grid-muted-text)', marginBottom: 6 }}>{t('debug.color')}</div>
            <div style={{ display: 'flex', gap: 6 }}>
              {DEBUG_GRID_PRESET_COLORS.map(color => (
                <button
                  key={color}
                  ...
                  style={{
                    ...
                    height: 24,
                    background: color,
                    border: settings.color === color ? '2px solid var(--debug-grid-preset-active-border)' : '1px solid var(--debug-grid-preset-border)',
                    borderRadius: 4,
                    cursor: 'pointer',
                    boxShadow: settings.color === color ? 'var(--debug-grid-preset-active-shadow)' : 'none',
                  }}
                />
              ))}
            </div>
          ...
          <div style={{ marginBottom: 10 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              ...
              />
              <span style={{ color: 'var(--debug-grid-muted-text)' }}>{t('debug.showMeasurements')}</span>
            </label>
          </div>

          {/* Presets */}
          <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${hexToRgba(settings.color, 0.2)}` }}>
            <div style={{ color: 'var(--debug-grid-muted-text)', marginBottom: 6, fontSize: 9 }}>{t('debug.presets')}</div>
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              <button
                onClick={() => setSettings({ ...settings, columns: 12, rows: 12, gutter: 16 })}
                style={{
                  padding: '4px 8px',
                  background: 'var(--debug-grid-preset-bg)',
          ...
          <div style={{ marginTop: 10, fontSize: 9, color: 'var(--debug-grid-subtle-text)' }}>
            {t('debug.ctrlClose')}
          </div>
        </div>
      )}
      ...
      <button
        ...
        style={{
          ...
          fontSize: 16,
          color: enabled ? settings.color : 'var(--debug-grid-button-muted-text)',
          cursor: 'pointer',
          zIndex: 100000,
          boxShadow: enabled 
            ? `0 4px 12px ${hexToRgba(settings.color, 0.4)}, 0 0 20px ${hexToRgba(settings.color, 0.2)}` 
            : 'var(--debug-grid-button-shadow)',
          pointerEvents: 'auto',
      ...
      <button
        ...
        style={{
          ...
          justifyContent: 'center',
          fontSize: 16,
          color: showSettings ? settings.color : 'var(--debug-grid-button-muted-text)',
          cursor: 'pointer',
          zIndex: 100000,
